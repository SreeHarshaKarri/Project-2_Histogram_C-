...
vector< vector<float> > normalize(vector< vector <float> > grid) {
	...
	vector<float> row;
  	...
  	height = grid.size();
  	width = row.size();
  	newGrid.resize(height, vector <float> (width, 0));
...
vector < vector <float> > blur(vector < vector < float> > grid, float blurring) {
	...
	signed int dx, dy;
	float center_prob, corner_prob, adjacent_prob, grid_value, scalar;
	vector<float> row;
	...
	float window[3][3] = {
				...
				{adjacent_prob, corner_prob, adjacent_prob},
				{corner_prob, adjacent_prob, corner_prob}
	};
		...
		for (j = 0; j < width; j++)
		{
			...
			for(dx = -1; dx < 2; dx++)
			{
				for(dy = -1; dy < 2; dy++)
				{
					...
					newGrid[m][n] = newGrid[m][n] + (scalar*grid_value);
				}
			}
		}
	}
